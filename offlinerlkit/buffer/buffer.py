...
class ReplayBuffer:
    def __init__(
        ...
        if self.save_penalty:
            self.penalties = np.zeros((self._max_size, 1), dtype=np.float32)
    ...
    def add_batch(
        ...
        if penalties is not None:
            self.penalties[indexes] = np.array(penalties).copy()
    ...
    def sample(self, batch_size: int) -> Dict[str, torch.Tensor]:
        ...
        if self.save_penalty:
            batch["penalties"] = torch.tensor(self.penalties[batch_indexes]).to(self.device)
        return batch
