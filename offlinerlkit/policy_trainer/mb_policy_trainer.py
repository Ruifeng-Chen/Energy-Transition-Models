...
class MBPolicyTrainer:
    ...
    def train(self) -> Dict[str, float]:
        ...
        for e in range(1, self._epoch + 1):
            ...
            for it in pbar:
                if num_timesteps % self._rollout_freq == 0:
                    ...
                    self.logger.log(
                        "num rollout transitions: {}, reward mean: {:.4f}, rollout time: {}".\
                            format(rollout_info["num_transitions"], rollout_info["reward_mean"], rollout_time)
                    )
                    for _key, _value in rollout_info.items():
                        self.logger.logkv_mean("rollout_info/"+_key, _value)
                    self.logger.logkv_mean("time/rollout_time", rollout_time)
                ...
                self.logger.logkv_mean("time/policy_train_time", train_time)
            ...
            self.logger.logkv("time/eval_time", eval_time)
        ...
        if hasattr(self.policy.dynamics, 'save'):
